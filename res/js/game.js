...
function playLevelVideo(level, callback) {
    const videoElement = document.getElementById('levelVideo');
    const videoSource = document.getElementById('videoSource');
    const menu = document.getElementById('menu');
    const intro = document.getElementById('intro'); 
    const introVideo = document.getElementById('introVideo');
    const canvas = document.getElementById('canvas'); // El canvas de juego

    // Configura el video según el nivel
    videoSource.src = `./res/video/NV${level}.mp4`;
    videoElement.style.display = 'block';
    menu.style.display = 'none';
    intro.style.display = 'none'; // Ocultar la intro
    introVideo.style.display = 'none'; // Ocultar video intro
    canvas.style.display = 'none'; // Ocultar el canvas mientras se reproduce el video del nivel

    videoElement.load();
    videoElement.play();

    // Escucha el evento para ocultar el video y mostrar el canvas
    videoElement.onended = () => {
        videoElement.style.display = 'none'; // Esconde el video
        canvas.style.display = 'block'; // Muestra el canvas
        callback(); // Inicia el nivel
    };
    
}

function startGame() {
    playLevelVideo(currentLevel, () => {});
    died = false;
    ...
    if (currentLevel === undefined || currentLevel < 0) {
        console.error("Error: currentLevel es inválido.", currentLevel);
    }
...
const deathSound = new Audio('./res/audio/death_sound.mp3');
deathSound.addEventListener('error', (e) => {
    ...
    function animation() {
        ...
        if (delta > interval) {
            ...
            if (allDoors[0].opened == true && allDoors[1].opened == true) {
                ...
                // Guardar los tiempos actualizados en el localStorage
                localStorage.setItem('completedLevelsTimes', savedTimes.join(','));
...
export { playGame, playLevelVideo };
export {currentLevel};